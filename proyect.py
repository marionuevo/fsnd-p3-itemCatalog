...
def newModel(style_id):
    ...
    if request.method == 'POST':
        ...
        return render_template('newModel.html', style_id=style_id)


# Edit a model
@app.route(
    '/style/<int:style_id>/model/<int:model_id>/edit', methods=['GET', 'POST'])
...
def deleteModel(style_id, model_id):
    ...
    if request.method == 'POST':
        ...
        return render_template('deleteModel.html', item=modelToDelete)
...
def showModelInfo(style_id, model_id):
    ...
    return render_template('modelInfo.html', model=model)
